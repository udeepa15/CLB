#!/usr/bin/env python3
"""
run_step3_dynamic_trial.py — Single Trial Validation for Dynamic Stackelberg QoS (ebpf_qos_dynamic.c + qos_controller.py).
Runs ONE HTTP baseline trial (QPS=50, conns=2, 10s, flood=0) with ebpf_qos_dynamic.c and qos_controller.py running.
Dumps raw Fortio JSON, eBPF map stats, and qos_controller_log.jsonl.
"""
...
def main():
    ...
    results_dir = os.path.join("results", "step3_dynamic_single_trial")
    os.makedirs(results_dir, exist_ok=True)

    log("Step 1: Setting up network topology & attaching ebpf_qos_dynamic.c...")
    run_cmd("./setup_topology.sh")
    run_cmd("./attach_ebpf.sh qos_dynamic")

    log("Step 2: Starting userspace Stackelberg QoS controller...")
    controller_log = os.path.join(results_dir, "qos_controller_log.jsonl")
    ...
    log("Step 4: Starting eBPF stats poller...")
    ebpf_stats_file = os.path.join(results_dir, "ebpf_stats_dynamic_baseline.jsonl")
    poller = subprocess.Popen(["python3", "collect_ebpf_stats.py", ebpf_stats_file])

    log("Step 5: Executing single Fortio HTTP trial (QPS=50, conns=2, 10s, flood=0)...")
    fortio_json = os.path.join(results_dir, "fortio_dynamic_http_baseline.json")
    ...
    log(f"Step 3 Single Trial Complete! Results saved in {results_dir}")
