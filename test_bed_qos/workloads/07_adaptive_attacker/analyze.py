#!/usr/bin/env python3
"""
analyze.py — Workload 07 3-Arm Ablation Analysis & Plot Generator.
"""
...
def main():
    ...
    print("=== Workload 07: Rational Adaptive Adversary 3-Arm Ablation Analysis ===")
    print(df.groupby("arch")[["p99_ms", "actual_qps"]].mean())

    plt.figure(figsize=(8, 5))
    sns.barplot(data=df, x="arch", y="p99_ms", palette="Set2")
    plt.title("Workload 07: 3-Arm Ablation under Rational Adaptive Adversary")
    plt.ylabel("Victim P99 Latency (ms)")
    plots_dir = os.path.join(latest_dir, "plots")
    os.makedirs(plots_dir, exist_ok=True)
    out_plot = os.path.join(plots_dir, "adaptive_3arm_p99.png")
