#!/usr/bin/env python3
"""
run_matrix.py — Workload 06: Unbounded Retry Storm Amplification Runner.

Executes trials using victim_server_retry.py (20% failure/delay injection)
under concurrent attacker flood (u20) to measure Request Amplification Factor.
...
def prepare_retry_victim_bundle(base_path, victim_id, protocol, port):
    ...
    # Copy modified victim_server_retry.py into container rootfs
    server_src = os.path.join(os.path.dirname(os.path.abspath(__file__)), "victim_server_retry.py")
    server_dst = os.path.join(bundle_name, "rootfs", "victim_server_retry.py")
    run_cmd(f"cp {server_src} {server_dst}")
    ...
    config["process"]["args"] = [
        "sh", "-c",
        f"exec python3 /victim_server_retry.py {protocol} {port}"
    ]
...
def run_single_trial(arch, rep, base_dir, raw_dir):
    trial_id = f"{arch}_retrystorm_rep_{rep}"
    log(f"--- Running Workload 06 Trial: {trial_id} ---")
    ...
    # 3. Start Modified Retry Victim Container
    ...
    time.sleep(1)
    prepare_retry_victim_bundle(base_dir, 1, "http", WORKLOAD_CONFIG["port_http"])
...
def main():
    parser = argparse.ArgumentParser(description="Workload 06: Retry Storm Runner")
    ...
    log(f"=== Workload 06: Retry Storm Benchmark (N={args.reps}, Total Trials: {len(trials)}) ===")
    ...
    with open(raw_csv, "w") as f:
        f.write("arch,protocol,pattern,rep,amplification_factor,p50_ms,p90_ms,p99_ms,p999_ms,actual_qps\n")
        for r in results_raw:
            f.write(f"{r['arch']},{r['protocol']},{r['pattern']},{r['rep']},{r['amplification_factor']},{r['p50_ms']},{r['p90_ms']},{r['p99_ms']},{r['p999_ms']},{r['actual_qps']}\n")

    log(f"Workload 06 Benchmark Run Complete! Summaries written to {run_dir}")
