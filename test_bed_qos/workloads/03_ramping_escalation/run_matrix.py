#!/usr/bin/env python3
"""
run_matrix.py — Workload 03: Ramping Escalation Attacker Runner.

Executes trials where flood intensity escalates monotonically every 2 seconds.
...
from workload_lib import SHARED_CONFIG, RampingAttackerThread, log, run_cmd, wait_for_port, parse_fortio_json

WORKLOAD_CONFIG = {
    **SHARED_CONFIG,
    "duration_sec": 16,
    "ramp_levels": ["u500", "u200", "u50", "u20", "u5", "u2", "flood"],
    "step_sec": 2.0
}
...
def run_single_trial(arch, rep, base_dir, raw_dir):
    trial_id = f"{arch}_ramping_rep_{rep}"
    log(f"--- Running Workload 03 Trial: {trial_id} ---")
    ...
    # 4. Start Ramping Attacker Thread
    attacker_thread = RampingAttackerThread(
        target_ip=WORKLOAD_CONFIG["victim1_ip"],
        target_port=WORKLOAD_CONFIG["port_http"],
        ramp_levels=WORKLOAD_CONFIG["ramp_levels"],
        step_sec=WORKLOAD_CONFIG["step_sec"]
    )
...
def main():
    parser = argparse.ArgumentParser(description="Workload 03: Ramping Escalation Runner")
    ...
    log(f"=== Workload 03: Ramping Escalation Benchmark (N={args.reps}, Total Trials: {len(trials)}) ===")
    ...
    log(f"Workload 03 Benchmark Run Complete! Summaries written to {run_dir}")
