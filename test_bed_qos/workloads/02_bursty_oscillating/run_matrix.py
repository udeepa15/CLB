#!/usr/bin/env python3
"""
run_matrix.py — Workload 02: Bursty Oscillating Attacker Traffic Runner.

Executes trials where the attacker alternates between low (u200) and high (flood)
intensity every 5s during a 20s run.
...
from workload_lib import SHARED_CONFIG, BurstyAttackerThread, log, run_cmd, wait_for_port, parse_fortio_json
...
def run_single_trial(arch, rep, base_dir, raw_dir):
    trial_id = f"{arch}_bursty_rep_{rep}"
    log(f"--- Running Workload 02 Trial: {trial_id} ---")
    ...
    # 4. Start Bursty Oscillating Attacker Thread
    attacker_thread = BurstyAttackerThread(
        target_ip=WORKLOAD_CONFIG["victim1_ip"],
        target_port=WORKLOAD_CONFIG["port_http"],
        level1=WORKLOAD_CONFIG["level1"],
        level2=WORKLOAD_CONFIG["level2"],
        interval_sec=WORKLOAD_CONFIG["burst_interval_sec"]
    )
...
def main():
    parser = argparse.ArgumentParser(description="Workload 02: Bursty Oscillating Attacker Runner")
    ...
    log(f"=== Workload 02: Bursty Oscillating Benchmark (N={args.reps}, Total Trials: {len(trials)}) ===")
    ...
    log(f"Workload 02 Benchmark Run Complete! Summaries written to {run_dir}")
