#!/usr/bin/env python3
"""
collect_ebpf_stats.py — Poll eBPF maps (lock_latency_hist and update_counter_map).

Computes diffs per 1-second polling interval to output hits/sec for bpf_map_update_elem
invocations on shared_global_key = 0 alongside lock latency histograms.
...
def extract_total_counter(counter_json):
    ...
    for entry in counter_json:
        values = entry.get("formatted", {}).get("values", [])
        for v in values:
            try:
...
def main():
    ...
    with open(out_file, 'w') as f:
        try:
            while True:
                ts = time.time()
                hist_data = get_bpf_map("lock_latency_hist")
