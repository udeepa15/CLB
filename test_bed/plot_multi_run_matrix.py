#!/usr/bin/env python3
"""
plot_multi_run_matrix.py — Plot Averaged P99 Tail Latency for Multi-Run (5 Reps) Matrix.

Reads results_summary_avg.csv from the multi-run benchmark directory,
plots side-by-side P99 tail latency comparisons for GRPC, TCP, and HTTP (and UDP if present),
and saves high-resolution publication charts to plots/p99_multi_run_avg.png.

Usage:
    python3 plot_multi_run_matrix.py
    python3 plot_multi_run_matrix.py --results-dir results/multi_run_N5_<timestamp>
"""
...
def plot_p99_tail_latency_matrix(df: pd.DataFrame, output_dir: str):
    """Generate side-by-side P99 Tail Latency comparison across GRPC, TCP, and HTTP."""
    target_protocols = ["grpc", "tcp", "http"]
    protocols = [p for p in target_protocols if p in df["protocol"].unique()]
    # Add any extra protocols (e.g. udp) if present
    ...
    for idx, proto in enumerate(protocols):
        ...
        for arch, color, marker, ls, label in [
            ("sidecar", COLOR_SIDECAR, "s", "--", "Sidecar Proxy Baseline"),
        ...
        ax.set_title(f"{proto.upper()} Protocol — Average P99 Tail Latency (N=5)", fontweight="bold", pad=10)
    ...
    fig.suptitle("Averaged Noisy Neighbor Contention: Sidecar vs Sidecarless eBPF (P99 Latency, N=5 Runs)", fontsize=14, fontweight="bold", y=0.98)
    plt.tight_layout(rect=[0, 0.02, 1, 0.95])

    out_file = os.path.join(output_dir, "p99_multi_run_avg.png")
    plt.savefig(out_file, bbox_inches="tight")
    print(f"Saved Averaged P99 Latency Plot: {out_file}")


def main():
    parser = argparse.ArgumentParser(description="Plot Averaged P99 Tail Latency for Multi-Run Benchmark Matrix")
    ...
    print(f"Reading averaged benchmark summary from: {os.path.abspath(summary_path)}")
    ...
    try:
        ...
        import shutil
        for p_file in glob.glob(os.path.join(plots_dir, "p99_multi_run_avg.png")):
            shutil.copy(p_file, res_plots_dir)
    except Exception:
        pass

    print("\nPlot generation complete! Generated plots/p99_multi_run_avg.png")
